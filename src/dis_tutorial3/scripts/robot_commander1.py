    ...
    def _navigation_result_callback(self, future):
        ...
        try:
            ...
            if status == GoalStatus.STATUS_SUCCEEDED:
                ...
                self.get_logger().warn(f"Navigation failed. Status: {status}, result: {future.result().result}")                
                # Handle aborted navigation
                if status == GoalStatus.STATUS_ABORTED:
                    # If we were approaching a person, try the alternative approach next time
                    if self.state == RobotState.APPROACHING_PERSON or self.state == RobotState.APPROACHING_RING:
                        self.get_logger().info("Navigation aborted during approach, will try alternative approach")
                    elif self.state == RobotState.RETURNING_TO_PLAN or self.state == RobotState.FOLLOWING_GLOBAL_PLAN:
                        # Jump ahead in the plan to try to recover
                        if self.global_path and self.path_index + 1 < len(self.global_path):
                            self.path_index += 1
                            self.get_logger().info(f"Navigation aborted, skipping to next waypoint {self.path_index}")
        except Exception as e:

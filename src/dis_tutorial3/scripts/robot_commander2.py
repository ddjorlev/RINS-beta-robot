...
class RobotState(Enum):
    ...
    APPROACHING_RING = 5
    RETURNING_TO_PLAN = 6
    PROCESSING_VISION = 7  # Add this new state
    RECOVERING = 8  # New recovery state
...
class RobotCommander(Node):
    ...
    def control_loop(self):
        ...
        try:
            # State timeout check (30 seconds max in any state)
            if time.time() - self.last_state_change > 30.0:
                self.get_logger().error(f"Timeout in {self.state.name}")
    ...
    def follow_global_plan(self):
        ...
        if time.time() - self.last_state_change > 20.0:

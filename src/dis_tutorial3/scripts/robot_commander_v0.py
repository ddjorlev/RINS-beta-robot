...
class RobotCommander(Node):
    def __init__(self):
        ...
        self.declare_parameter('approach_distance', 0.6)  # Distance to approach person/ring
        ...
        self._current_goal_state = None  # Tracks what state the goal was for
        self._navigating = True
        self._current_goal_state = self.state  # Save what this goal was for
        ...
        self.detection_queue = deque()  # Queue of DetectedObject
        self.seen_objects = {}  # Key: (rounded x, y), Value: timestamp or object ID
        self.detection_radius = 0.3  # meters, for position matching
    ...
    def people_poses_callback(self, msg: PoseArray):
        ...
        for pose in msg.poses:
            ...
            # Convert quaternion to forward vector (x-axis in quaternion space)
            normal = np.array([
                1 - 2*(qy*qy + qz*qz),
                2*(qx*qy + qw*qz),
                2*(qx*qz - qw*qy)
            ...
            # Keep only 2D components for navigation
            normal_2d = normal[:2]
            norm = np.linalg.norm(normal_2d)
            if norm > 0.001:
                normal_2d = normal_2d / norm
            
            # Update 3D normal with normalized 2D components
            ...
            rounded_pos = tuple(np.round(position[:2], 1))  # Round for easier matching

            # Only enqueue if not already seen
            if rounded_pos not in self.seen_objects:
                self.seen_objects[rounded_pos] = current_time
                self.detection_queue.append(DetectedObject(
                    position=position,
                    normal=normal,
                    object_type="person",
                    last_seen=current_time
    ...
    def control_loop(self):
        ...
        elif self.detection_queue:
            next_obj = self.detection_queue.popleft()
            if next_obj.object_type == "person":
                self.detected_objects = [next_obj]  # override to make it look like freshly detected
                self.state = RobotState.APPROACHING_PERSON
            elif next_obj.object_type == "ring":
                self.detected_objects = [next_obj]
    ...
    def wait_for_global_path(self):
        ...
        if self.global_path and self.state == RobotState.WAITING_FOR_PATH:
    ...
    def approach_person(self):
        """Approach a detected person with smarter vector calculation."""
        ...
        if recent_people:
            ...
            try:
                ...
                if dist_to_person > 0.01:
                    robot_to_person = robot_to_person / dist_to_person  # Normalize
                
                # Get the person's normal vector (direction they're facing)
                normal_2d = person.normal[:2]
                normal_norm = np.linalg.norm(normal_2d)
                
                if normal_norm > 0.01:
                    normal_2d = normal_2d / normal_norm  # Normalize
                    
                    # Calculate dot product to see if person is facing toward or away from robot
                    dot_product = np.dot(normal_2d, -robot_to_person)
                    
                    # If dot product is positive, person is generally facing the robot
                    # If negative, person is facing away from robot
                    
                    if dot_product > 0:
                        # Person is facing toward robot - approach from current direction
                        # but maintain proper distance
                        approach_vec_2d = -robot_to_person  # From person toward robot
                        self.get_logger().info("Person is facing robot, approaching from current position")
                    else:
                        # Person is facing away from robot - try to get in front of them
                        # but from the side we're already on
                        
                        # Calculate perpendicular vectors to the normal
                        perp1 = np.array([-normal_2d[1], normal_2d[0]])  # 90 degrees clockwise
                        perp2 = np.array([normal_2d[1], -normal_2d[0]])  # 90 degrees counterclockwise
                        
                        # Choose the perpendicular vector that's closer to our current approach
                        dot1 = np.dot(perp1, robot_to_person)
                        dot2 = np.dot(perp2, robot_to_person)
                        
                        # Use the perpendicular that's on the same side as the robot
                        side_vec = perp1 if dot1 > dot2 else perp2
                        
                        # Blend between side approach and front approach to avoid walls
                        # More weight to side approach (safer) than front approach (might hit walls)
                    ...
                    # Normal isn't usable, use robot-to-person
                ...
                # Default approach - towards the person from current position
                approach_vec_2d = np.array([1.0, 0.0])  # Default direction
                
            # Use multiple approach distances to find navigable position
            distances_to_try = [
                # self.approach_distance * 2.0,  # First try double distance
                # self.approach_distance * 1.5,  # Then try 1.5x distance
                self.approach_distance,        # Then normal distance
                # self.approach_distance * 0.7,  # Then try smaller distances
            ]
            
            self.get_logger().info(f"Number of distances to try: {len(distances_to_try)}")
            navigation_success = False
            
            for distance in distances_to_try:
                # Calculate approach position with the current distance
                target_pos = person_pos_2d + approach_vec_2d * distance
                
                # Log approach information
                self.get_logger().info(f"Person position: {person_pos_2d}")
                self.get_logger().info(f"Using approach vector: {approach_vec_2d}")
                self.get_logger().info(f"Trying approach at distance {distance}m: {target_pos}")
                
                # Try to navigate to this position
                if self.navigate_to(target_pos):
                    self.get_logger().info(f"Successfully approaching at distance {distance}m")
                    navigation_success = True
                    break
                else:
                    self.get_logger().warn(f"Failed to approach at distance {distance}m, trying next distance")
            
            # Move to returning to plan regardless of success/failure
            if not navigation_success:
                self.get_logger().warn("All approach attempts failed, returning to plan")
            
            # Even if navigation fails or succeeds, proceed to return to plan
            self.state = RobotState.RETURNING_TO_PLAN
        else:
            # No person detected, return to plan
    ...
    def is_position_valid(self, position):
        """Check if a position is likely to be navigable"""
        try:
            # First check: Is it within a reasonable distance?
            transform = self.tf_buffer.lookup_transform(
                "map", "base_link", rclpy.time.Time(), rclpy.duration.Duration(seconds=1.0))
            robot_pos = np.array([transform.transform.translation.x, transform.transform.translation.y])
            
            # Check if it's too far from the robot's current position
            distance_to_robot = np.linalg.norm(position - robot_pos)
            if distance_to_robot > 10.0:  # Max 5m from robot for safety
                self.get_logger().warn(f"Target position too far: {distance_to_robot:.2f}m")
                return False
                
            # # Check if it's too close to the robot
            # if distance_to_robot < 0.15:  # Too close (less than 15cm)
            #     self.get_logger().warn(f"Target position too close: {distance_to_robot:.2f}m")
            #     return False
            
            # Basic boundary checks (if you know the map dimensions)
            # For example, if your map is roughly 10x10 meters:
            # if position[0] < -5.0 or position[0] > 5.0 or position[1] < -5.0 or position[1] > 5.0:
            #     self.get_logger().warn(f"Target position outside map bounds: {position}")
            #     return False
            
            return True
        except Exception as e:
            self.get_logger().error(f"Error checking position validity: {e}")
            return False  # Be conservative - if we can't verify, don't navigate
    ...
    def return_to_global_plan(self):
        ...
        # Ensure path_index is valid and within bounds
        if self.path_index >= len(self.global_path):
            self.get_logger().warn(f"Path index {self.path_index} out of bounds, limiting to last point")
            self.path_index = len(self.global_path) - 1
        
        if self.path_index < len(self.global_path):
            target = self.global_path[self.path_index]
            
            # Log the return information
            try:
                transform = self.tf_buffer.lookup_transform(
                    "map", "base_link", rclpy.time.Time(), rclpy.duration.Duration(seconds=1.0))
                robot_pos = np.array([transform.transform.translation.x, transform.transform.translation.y])
                distance = np.linalg.norm(robot_pos - target)
                self.get_logger().info(f"Distance to return point: {distance:.2f}m")
            except Exception as e:
                self.get_logger().warn(f"Could not calculate distance to return point: {e}")
            
            # Try to navigate to the point
            success = self.navigate_to(target)
            if success:
                self.get_logger().info(f"Returning to global plan at point {self.path_index}")
                self.state = RobotState.FOLLOWING_GLOBAL_PLAN
            else:
                # If navigation fails, try the next point
                self.path_index += 1
                if self.path_index < len(self.global_path):
                    self.get_logger().warn(f"Return navigation failed, trying next point {self.path_index}")
                    self.navigate_to(self.global_path[self.path_index])
                    self.state = RobotState.FOLLOWING_GLOBAL_PLAN
                else:
                    self.get_logger().error("Could not return to global plan")
                    self.state = RobotState.IDLE
        else:
            self.get_logger().info("Reached end of global plan")
        self.state = RobotState.IDLE
    ...
    def navigate_to(self, target):
        ...
        if not self.is_position_valid(target):
            self.get_logger().warn(f"Target position {target} seems invalid, aborting navigation")
            return False
        ...
        # Calculate meaningful orientation
        try:
            transform = self.tf_buffer.lookup_transform(
                "map", "base_link", rclpy.time.Time(), rclpy.duration.Duration(seconds=1.0))
            robot_pos = np.array([transform.transform.translation.x, transform.transform.translation.y])
    ...
    def _navigation_result_callback(self, future):
        ...
        try:
            ...
            # Ignore if this result is no longer relevant
            if self.state != self._current_goal_state:
                self.get_logger().info(
                    f"Ignoring outdated result: current state is {self.state.name}, goal was sent from {self._current_goal_state.name}"
                )
                return
            ...
            if status == GoalStatus.STATUS_SUCCEEDED:
                ...
                if status == GoalStatus.STATUS_ABORTED and self.state in [RobotState.FOLLOWING_GLOBAL_PLAN, RobotState.RETURNING_TO_PLAN]:
                    self.path_index += 1

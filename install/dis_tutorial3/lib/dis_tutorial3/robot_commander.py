...
def main(args=None):
    ...
    for i, (x, y, yaw) in enumerate(waypoints):
        rc.info(f"Navigating to waypoint {i+1}/{len(waypoints)}: ({x}, {y})")
        ...
        while not rc.isTaskComplete():
            rc.info(f"Moving to waypoint {i+1}...")
            time.sleep(1)
            
        rc.info(f"Reached waypoint {i+1}!")
        
        # Optional: spin at each waypoint to look around
        if i < len(waypoints) - 1:  # Don't spin at the last waypoint
            rc.info("Spinning to look around...")
            rc.spin(6.28)  # Spin 90 degrees
            while not rc.isTaskComplete():
                time.sleep(0.5)
